...
export const getStatusbyIdAnimal = async (req: Request, res: Response) => {
	try {
		const { idAnimal } = req.params
		const status = await AppDataSource.getRepository(StatusAnimal).find({ relations: ["animal"], where: { animal: { id: idAnimal } } })
		res.json({
			ok: true,
			message: "Lista de estados de animales",
			status: status
